...
import { resetStaffPassword, createStaffSession } from "@/lib/staff-session";
...
async function submitReset(token: string, formData: FormData) {
  ...
  if (!password || password !== confirmPassword) {
    redirect(`/staff/reset-password?token=${token}&error=mismatch`);
  }

  const staff = await resetStaffPassword(token, password);
  if (!staff) {
    redirect(`/staff/reset-password?token=${token}&error=invalid`);
  }
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center bg-zinc-50 font-sans dark:bg-black">
        <main className="w-full max-w-sm rounded-2xl border border-black/[.08] bg-white p-8 dark:border-white/[.145] dark:bg-[#0a0a0a]">
          ...
          <Link href="/staff/forgot-password" className="text-sm text-zinc-500 hover:underline dark:text-zinc-400">
            Request a new link
          </Link>
        </main>
      </div>
      ...
      <main className="w-full max-w-sm rounded-2xl border border-black/[.08] bg-white p-8 dark:border-white/[.145] dark:bg-[#0a0a0a]">
        ...
        {error === "invalid" && (
          <p className="mb-4 rounded-lg bg-red-100 px-3 py-2 text-sm text-red-800 dark:bg-red-950 dark:text-red-300">
            ...
            <Link href="/staff/forgot-password" className="underline">
              Request a new one
            </Link>
            .
          </p>
