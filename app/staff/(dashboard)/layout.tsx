...
export default async function StaffDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-full flex-1 bg-zinc-50 font-sans dark:bg-black">
      <aside className="no-print sticky top-0 flex h-screen w-64 shrink-0 flex-col border-r border-black/[.08] bg-white dark:border-white/[.145] dark:bg-[#0a0a0a]">
          ...
          {sections.map((section) => {
            ...
            return (
              <div key={section.label} className="flex flex-col gap-1">
                <span className="px-3 text-xs font-medium tracking-wide text-zinc-400 uppercase dark:text-zinc-500">
                  {section.label}
                </span>
