...
async function createStaff(formData: FormData) {
  ...
  if (!staff || !canManageStaff(staff.role) || !staff.accountId) return;
  ...
  if (!warehouse || warehouse.accountId !== staff.accountId) return;
...
async function updateStaff(staffId: string, formData: FormData) {
  ...
  if (!target || target.accountId !== staff.accountId || !isAssignableRole(target.role)) return;

  const roleRaw = String(formData.get("role") ?? "");
  const role = isAssignableRole(roleRaw) ? roleRaw : null;
  const warehouseId = String(formData.get("warehouseId") ?? "").trim();
  const additionalWarehouseIds = formData
    .getAll("additionalWarehouseIds")
    .map(String)
...
export default async function StaffTeamPage({
  ...
  if (!canManageStaff(staff.role) || !staff.accountId) {
  ...
  const [members, warehouses] = await Promise.all([
    ...
    prisma.warehouse.findMany({ where: { accountId: staff.accountId }, orderBy: { name: "asc" } }),
  ]);
        ...
        {members.length === 0 ? (
          ...
          <ul className="flex flex-col gap-3">
            {members.map((m) => {
              ...
              const editable = isAssignableRole(m.role);
              return (
                <li
                  key={m.id}
                    ...
                    <form action={boundUpdate} className="flex flex-wrap items-end gap-2">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <select
                          id={`warehouseId-${m.id}`}
                          name="warehouseId"
                          defaultValue={m.warehouseId ?? undefined}
                          className="h-9 rounded-lg border border-black/[.08] bg-white px-2 text-sm text-black dark:border-white/[.145] dark:bg-black dark:text-zinc-50"
                        >
                          {warehouses.map((w) => (
                            <option key={w.id} value={w.id}>
