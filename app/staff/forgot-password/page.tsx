...
import { requestStaffPasswordReset } from "@/lib/staff-session";
...
async function requestReset(formData: FormData) {
  ...
  if (email) {
    ...
    await requestStaffPasswordReset(email, `${protocol}://${host}`);
  }

  redirect("/staff/forgot-password?sent=1");
}
      ...
      <main className="w-full max-w-sm rounded-2xl border border-black/[.08] bg-white p-8 dark:border-white/[.145] dark:bg-[#0a0a0a]">
        ...
        {sent ? (
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            If that email belongs to a staff account, we&rsquo;ve sent a link to reset the password. The link
        ...
        <Link href="/staff/login" className="mt-4 block text-sm text-zinc-500 hover:underline dark:text-zinc-400">
          ← Back to login
        </Link>
      </main>
    </div>
