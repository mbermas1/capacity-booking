...
export async function GET(request: NextRequest) {
  ...
  const report = await computeUtilizationReport(startInclusive, end, { carrierName, warehouseId: staff.warehouseId });

  const rows = report.flatMap((dock) =>
    dock.cells.map((c) => [
      dock.dockName,
      c.date,
      c.hour,
      c.bookingCount,
      c.bookedMinutes,
      Math.round(c.utilization * 10000) / 100,
    ]),
  );

  const csv = toCsv(["Dock", "Date", "Hour", "Bookings", "Booked Minutes", "Utilization %"], rows);

  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv",
      "Content-Disposition": `attachment; filename="utilization-report-${startParam}-${endParam}.csv"`,
    },
  });
}
