  ...
  try {
    const response = await fetch("https://api.emailit.com/v2/emails", {
      ...
      body: JSON.stringify({
        from: fromEmail,
        to,
        subject: `Confirm your dock booking request — ${context.dockName}`,
        html: `<p>Confirm your booking request for ${context.dockName}:</p><p><a href="${verifyUrl}">${verifyUrl}</a></p><p>This link expires in 30 minutes.</p>`,
      }),
    });
